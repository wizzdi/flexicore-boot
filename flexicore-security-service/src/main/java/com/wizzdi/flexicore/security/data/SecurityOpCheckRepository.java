...
@Extension
@Repository
public class SecurityOpCheckRepository implements Plugin {
	 ...
	 */
	public boolean checkRole(IOperation securityOperation, IUser securityUser, Access access) {
		...
		Predicate rolesPredicate = cb.and(
				...
				cb.equal(roleToBaseClass.get(RoleToBaseclass_.securedId), securityOperation.getId()),
	...
	public boolean checkUser(SecurityOperation securityOperation, SecurityUser securityUser, Access access) {
		...
		Predicate directPredicate = cb.and(
				...
				cb.equal(users.get(SecurityUser_.id), securityUser.getId()),
				cb.equal(direct.get(UserToBaseclass_.securedId), securityOperation.getId()),
